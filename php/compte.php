...
<body>
	...
	<main> <!-- On place dans cette balise le contenue de la page "compte" -->
		
		<?php
			
			include("maj_session.php");
			include("connexion_db.php");
			$requete = "select * from membres where ID=".$_SESSION['ID'];
		...
		<div>
			...
			<form method="post" action="modifier.php">
				...
				<fieldset> <!-- Cette balise contient la première case de contact dans laquelle l'utilisateur peut donner ses informations personnelles -->
					...
					<br>
					<label>Genre:</label>
					<input type="radio" name="genre" id="Feminin" value="Feminin" required <?php if($genre=='Feminin')echo'checked';?>>
					<label for="mme">Madame</label>
					<input type="radio" name="genre" id="Masculin" value="Masculin" required <?php if($genre=='Masculin')echo'checked';?>>
					...
					<input type="email" name="courriel" id="courriel" value=<?php echo $row['Courriel'];?> required />
					<br><br>
					...
					<input type="reset">
					<br><br>
					<button><a class="button" href="logout.php">Déconnexion</a></button>
					<?php if($_SESSION["admin"]=="Admin"){echo '<button><a class="button" href="admin.php">Admin</a></button>';
				...
				<?php
					...
					if(isset($_SESSION["update"]) && $_SESSION["update"] == 1) {
						echo '<p class="update">Infos modifié</p>' ;
						$_SESSION["update"] = 0;
					}
				?>
